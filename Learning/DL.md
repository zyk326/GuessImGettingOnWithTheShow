...
#### **跳转**  
[数据](#数据) [线性代数](#线性代数) [矩阵计算](#矩阵计算) [自动求导](#自动求导) [线性回归+基础优化算法](#线性回归基础优化算法) [Softmax回归+损失函数+图片分类数据集](#softmax回归损失函数图片分类数据集)  [模型选择+过拟合and欠拟合](#模型选择过拟合and欠拟合)  [权重衰退](#权重衰退)  [丢弃法](#丢弃法)  [数值稳定性+模型初始化和激活函数](#数值稳定性模型初始化和激活函数)  [pytorch神经网络基础](#pytorch神经网络基础)  [卷积](#卷积)  [卷积层的多输入多输出通道](#卷积层的多输入多输出通道)  [池化层](#池化层)  [lenet](#lenet)  [alexnet](#alexnet)  [使用块的网络vgg](#使用块的网络vgg)  [ninnetwork-in-network](#ninnetwork-in-network)  [googlenet](#googlenet)  [批量归一化](#批量归一化)  [resnet](#resnet)  [cpu和gpu](#cpu和gpu)  [数据增广](#数据增广)  [微调](#微调)  [物体检测](#物体检测)  [物体检测算法r-cnn-ssd-yolo](#物体检测算法r-cnn-ssd-yolo)  [语义分割](#语义分割)  [转置卷积操作子](#转置卷积操作子)  [全连接卷积神经网络fcn](#全连接卷积神经网络fcn)  [样式迁移](#样式迁移)  [序列数据](#序列数据)  [rnn-循环神经网络](#rnn-循环神经网络)  [门控循环单元-gru](#门控循环单元-gru)  [长短期记忆网络lstm](#长短期记忆网络lstm)  [编码器-解码器结构](#编码器-解码器结构)  [seq到seq-序列到序列学习](#seq到seq-序列到序列学习)  [注意力机制](#注意力机制)  [注意力分数](#注意力分数)  [使用注意力的seq2seq](#使用注意力的seq2seq)  [自注意力机制](#自注意力机制)  [transformer](#transformer)
...
Faster R-CNN和Mask R-CNN是在最求高精度场景下的常用算法,例如无人车领域.  
...
转置卷积则可以用来增大输入高宽.  
[转置卷积的内容可以看这里](https://www.cnblogs.com/harrymore/p/17254833.html)

## 全连接卷积神经网络(FCN)

它是深度神经网络来做语义分割的奠基性的工作.  
...
![](./img/base-cnn-trans-style.png)

## 序列数据

实际中很多数据是有时序结构的,比如电影的评价.  

自回归模型用自身过去数据来预测未来.  

马尔科夫假设:当前数据只跟τ个过去数据点相关.  
...
![](./img/CNN.png)  
![](./img/CNNsum.png)
RNN要做很多的乘,所以它需要有梯度剪裁的动作,防止梯度爆炸.  

## 门控循环单元 GRU

关注一个序列的时候,不是每个观察值都是同等重要的,只记住相关的观察需要:  
* 更新门:能关注的机制.  
* 重置门:能遗忘的机制.  
...
![](./img/endecode-format.png)    
* 使用编码器-解码器架构的模型,编码器负责表示输入,解码器负责输出,而且解码器可以有额外的输入.    
...
一个额外的点:束搜索,每次取最好的k个节点保留,k为1是贪心,k为n不是暴力.   
...
它显示地加入了query这个东西,根据查询去找比较感兴趣的东西.  

![](./img/attention.png)  

非参注意力:我通过一些方法,比如平均池化,来计算一个值,利用这个值作参考,比如与之相近的值来做计算,起到注意力的作用.  

exp把值搞到>=0.  

![](./img/attention-summarize.png)

## 注意力分数

![](./img/ATTENSCORE.png)   

![](./img/attention-scores.png)  

可以根据实际网络来设置监听的key和value.  

## 使用注意力的seq2seq  

动机是说我要翻译的当前位置与源句子的某一个位置更相关.   

![](./img/seqwithatten.png)  

![](./img/seqattsum.png)  

重点的是在每一次的最后的隐信息作为query,去找符合的编码器的内容,来更新context.   

![](./img/pointatt.png)

## 自注意力机制  

这个东西来选key, value, 和 query.  
