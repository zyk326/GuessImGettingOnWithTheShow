...
- [**第三部分：HALCON 案例实战**](#第三部分halcon-案例实战)
- [**第四部分：VisionMaster 专题**](#第四部分visionmaster-专题)
- [**第五部分：VisionMaster 案例实战**](#第五部分visionmaster-案例实战)
...
# 第五部分：VisionMaster 案例实战

---

## 目录（第五部分）

1. [标定与生产双流程案例](#1-标定与生产双流程案例)
2. [二维码定位与识别案例](#2-二维码定位与识别案例)
...
以一个实际项目（立讯吉安无线手表充电器标定）为例，需要在同一个 `.sol` 方案文件中同时包含**标定流程**和**生产流程**，标定完成后直接切换生产，实现标定→生产的无缝衔接。

**检测目标**：
1. 建立像素坐标→物理坐标的映射，生产时实时计算目标偏移量并输出
2. 计算物料的旋转角度（通过充电器头中心点与线圈中心点的连线）
...
```
图像源(9个位置) → 高精度匹配 → N 点标定 → 生成标定文件 (.xml)
```

| 步骤 | 模块 | 说明 |
|------|------|------|
| **图像源** | 图像采集 | 加载九个位置的物料图片，必须拍正确 |
| **高精度匹配** | 高精度匹配 | 以匹配框中心作为标定点输入，一致性是关键 |
| **N 点标定** | N 点标定 | 按点逐次触发输入；图像点取高精度匹配框中心；9 点标定平移 3 次 |
| **输出** | 标定文件 | 生成 `.xml` 标定文件 |

**标定走线要求**：
- N 点标定出来的走线必须是**正田字格**，否则生成的标定文件误差大
- 物理坐标最好是**规则的 XY 步长**（如同为 30mm 或 20mm）
...
生产流程分**两个分支**，分别获取运行点和基准点，通过差值计算偏移量：

```
┌── 分支①：获取运行点 ──────────────────────┐
│  图像源 → 高精度匹配 → 标定转换 → 运行点坐标  │
└────────────────────────────────────────────┘
                    ↓
     偏移量 = 运行点坐标 − 基准点坐标
                    ↓
┌── 分支②：获取基准点（示教） ─────────────────┐
│  示教图像 → 高精度匹配 → 标定转换 → 基准点坐标 │
└────────────────────────────────────────────┘
```

| 分支 | 用途 | 说明 |
|------|------|------|
| **分支① 运行点** | 当前产品实际位置 | 实时采集图像 → 高精度匹配定位 → 标定转换得到物理坐标 |
| **分支② 基准点** | 理想位置（示教点） | 通常用**中心点**做示教，通过标定转换得到固定的基准物理坐标 |
...
```
计算(运行点−基准点) → 条件检测(匹配状态) → 格式化(三项输出) → 使能输出
```

#### 条件检测

在计算完偏移量后，增加条件检测模块，检测两个高精度匹配模块的执行状态（匹配是否成功、分数是否达标）。
...
| 输出项 | 内容 | 说明 |
|--------|------|------|
| **第一项** | 条件检测结果 | OK / NG，表示两个匹配是否都成功 |
| **第二项** | ΔX（dx） | 运行点与基准点的 X 方向物理偏移 |
| **第三项** | ΔY（dy） | 运行点与基准点的 Y 方向物理偏移 |

#### 输出设置使能

在流程标签左侧的**扳手**（属性设置）→ **输出设置** 中，将格式化内容使能，才能正确输出数据到上位机。  
**注意！！！** 软件与PLC交互result不可以有0，所以数字交互是用1开始的。  

#### 偏移验证

测试标定图像的格式化坐标时，将输出的坐标值与选定的物理偏移量对比：

| 检查项 | 正常 | 异常 |
|--------|------|------|
| 格式化坐标 vs 物理偏移 | 一致或差值在允许范围 | 差异较大 |
| 处理方式 | 继续生产 | 检查标定图片中的物料位置，重新标定 |

---

### 1.5 旋转角度计算

本案例需要在获取偏移量的同时，计算物料的旋转角度。

#### 原理

以充电器头的中心点和线圈处的中心点，两点连线作为该物料的标准线；来料时计算来料的线与基准线做角度测量。

```
充电器头中心点 →┐
                ├── 两点连线 → 标准线 → 与基准线比较 → 返回角度
线圈处中心点   →┘
```

#### 找线圈中心点的关键步骤

线圈处的中心点不易直接定位，通过 VisionMaster 模块组合实现：

| 步骤 | 模块 | 说明 |
|------|------|------|
| **① 图像预处理** | 图像预处理 | 彩色图转灰度，增强对比度 |
| **② 形态学处理** | 形态学处理 | 先用腐蚀把单独的线去掉，再用开运算分离连接区域 |
| **③ Blob 分析** | Blob 分析 | 找到充电器头和线圈两个区域 |
| **④ 脚本处理** | 脚本模块 | 通过脚本将充电器头区域去除，只保留线圈的坐标 |
...
| 文件 | 用途 | 说明 |
|------|------|------|
| `标定_带旋转角度.sol` | 视觉方案文件 | 包含标定流程和生产流程的完整配置 |
| `.xml` 标定文件 | 标定参数文件 | N 点标定生成的像素↔物理坐标映射参数 |

> `.sol` 和 `.xml` **缺一不可**，现场部署时需两个文件一并拷贝到对应目录。

---
### 1.7 流程优化：分支模块替代条件跳过

在原流程中，某些被条件判断跳过的模块实际上仍然参与了执行周期。可以改用**分支模块**来优化：把被条件判断跳过的流程直接用分支绕过去，减少不必要的模块执行，提升整体处理速度。
...
在产品追踪与追溯场景中，需要通过视觉识别产品上的二维码信息，并根据工位状态输出分级结果。

**检测目标**：定位二维码区域 → 识别内容 → 分情况输出标志位
...
```
读入图像 → 高精度匹配(定位二维码区域) → 图像归一化(ROI直方图归一化)
    → 二维码识别 → 脚本(分多种情况) → 格式化输出
```

| 步骤 | 模块 | 说明 |
|------|------|------|
| **高精度匹配** | 高精度匹配 | 将二维码区域定位到 |
| **图像归一化** | 图像归一化 | 对高精匹配的 ROI 做直方图归一化，消除光照差异 |
| **二维码识别** | 二维码识别 | 读取二维码内容 |
| **脚本** | 脚本模块 | 根据识别结果分出多种情况，赋值标志位 |
| **格式化输出** | 格式化 | 按指定格式输出结果 |
...
| **1** | 充电盘位置正常（检测到且处于左边） |
| **2** | 未定位到充电盘 |
| **3** | 充电盘位置错误（检测到且处于右边，物料放反） |
| **4** | 充电盘已翻面 |

**输出格式**：`result = <标志位>,<dx>,<dy>,<a>`
