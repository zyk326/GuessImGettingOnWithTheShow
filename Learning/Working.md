...
- 封装了**坐标转换矩阵**的核心参数
- 包含：标定矩阵、旋转中心、标定误差、像素精度、坐标系信息等
- **使用方**：VisionMaster 内部与坐标转换相关的算法模块加载执行
- 工作流程：方案运行时，定位模块读取 .xml → 将相机识别的像素坐标 → 实时转换为机器人物理坐标

> **类比**：.iccal 是"校准相机镜头"，确保照片真实；.xml 是"建立翻译词典"，把像素点翻译成机器人能理解的物理坐标。
